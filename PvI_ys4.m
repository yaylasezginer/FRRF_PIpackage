...
function [l, ps, ps_err, pmax,ci_pmax, ek, ci_ek, alpha, ci_a, beta, ci_b, R2, mused, modelfit]= PvI_ys4(light, pvar, calc_no, mused)
...
initial_guess = [pmax_guess alpha_guess nan; 1.1*pmax_guess alpha_guess 0.5]; % [pmax, alpha, beta]
lb = [0.1*pmax_guess; 0; 0]; % lower bounds
ub = [10*pmax_guess; 10*alpha_guess; 10]; % upper bounds

if numel(light(~bad_data)) < 5 % min points for curve fit w 3 params
    [pmax,ci_pmax,ek, ci_ek, alpha, ci_a, beta, ci_b, R2,mused]= deal(nan);
    ...
    for i = 1:numel(model)
        ...
        [params,stats]= fit(l(~bad_data),ps(~bad_data),ft,'startpoint',initial_guess(i,~nonan),'lower',lb(~nonan),'upper',ub(~nonan));
        %[params,stats]= fit(light_clean,pvar_clean,ft,'startpoint',initial_guess(i,~nonan),'lower',lb(~nonan),'upper',ub(~nonan));
...
if beta1 <= 0
    ...
    plot(light_clean,pvar_clean,'.','markeredgecol',[.7 .7 .7]),  %plots all of the individual data points for each light step
    %c = {'r--', 'b--', 'c--','g--'};
    %for i = 1:numel(model)
